	...
	: QObject(parent)
{
	...
	connect(d->client, SIGNAL(actionMakeCall(const Jid &)),SLOT(actionMakeCall(const Jid &)));
#ifdef FILETRANSFER
...
void PsiAccount::setUserAccount(const UserAccount &_acc)
{
	...
	hostPortOverride["ya.ru"]          = HostPortOverride("xmpp.yandex.ru",      5222, UserAccount::SSL_Yes);
