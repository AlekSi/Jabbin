	...
	, commitTimer_(0)
{
	...
	connect(contactList, SIGNAL(addedContact(PsiContact*)), SLOT(addContact(PsiContact*)));
	connect(contactList, SIGNAL(removedContact(PsiContact*)), SLOT(removeContact(PsiContact*)));
	connect(contactList, SIGNAL(beginBulkContactUpdate()), SLOT(beginBulkUpdate()));
	connect(contactList, SIGNAL(endBulkContactUpdate()), SLOT(endBulkUpdate()));
}
