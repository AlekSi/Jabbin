	...
	: TabbableWidget(j.bare(), pa, tabManager)
{
	...
	// Common actions
	d->act_send = new QAction(tr("Send"), this);
...
{
	return PsiOptions::instance()->getOption("options.ui.muc.enabled").toBool();
}
...
{
	if (!d->act_send->isEnabled()) {
		return;
	}

	if (ui_.mle->chatEdit()->text().trimmed().isEmpty()) {
...
void GCMainDlg::chatEditCreated()
{
	...
	ui_.mle->chatEdit()->setSendAction(d->act_send);
