...
bool ChatDlg::readyToHide()
{
	...
	if (contactChatState_ == XMPP::StateComposing || contactChatState_ == StateInactive) {
...
#ifdef YAPSI
			dispNick_ = Ya::contactName(dispNick_, userStatus.userListItem->jid().full());
#endif
			nicksChanged();
			invalidateTab();
