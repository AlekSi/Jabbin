...
public:
	Iconset system;
#ifdef YAPSI
	Iconset* yaEmoticonSelectorIconset_;
#endif
...
#ifdef YAPSI
	{
		d->yaEmoticonSelectorIconset_ = 0;

		Q3PtrListIterator<Iconset> it(emoticons);
		Iconset* iconset;
		while ((iconset = it.current()) != 0) {
			++it;
			QFileInfo fi(iconset->fileName());
			if (fi.fileName() == "yaemomidget") {
				d->yaEmoticonSelectorIconset_ = iconset;
				break;
			}
		}

		if (!d->yaEmoticonSelectorIconset_) {
			d->yaEmoticonSelectorIconset_ = new Iconset();
			if (!d->yaEmoticonSelectorIconset_->load(d->iconsetPath("emoticons/yaemomidget"))) {
				qWarning("Unable to load yaemomidget iconset");
			}
		}
	}
#endif
...
{
	return d->yaEmoticonSelectorIconset_;
}
#endif
